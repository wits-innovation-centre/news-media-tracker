...
async function renameWorkspace(workspaceId: string, name: string, description?: string, iconPath?: string): Promise<void> {
  ...
  const trimmedName = name.trim()
  if (!trimmedName) throw new Error("Workspace name is required.")
  await getDb().execute(`UPDATE workspaces SET name = ?, description = ?, icon_path = COALESCE(?, icon_path) WHERE id = ?`, [trimmedName, description?.trim() || null, iconPath?.trim() || null, scopedWorkspaceId])
}

async function setWorkspaceTemplateGroup(workspaceId: string, templateGroupId?: string): Promise<void> {
  const scopedWorkspaceId = normalizeWorkspaceId(workspaceId)
  await getDb().execute(`UPDATE workspaces SET template_group_id = ? WHERE id = ?`, [templateGroupId?.trim() || null, scopedWorkspaceId])
}
...
async function loadActiveSchemas(workspaceId: string = getActiveWorkspaceId()) {
  ...
  const metadataRows = await db.query("SELECT schema_id, metadata FROM schema_metadata WHERE workspace_id = ?", [scopedWorkspaceId])
...
async function saveSchemaWorkspace(groups: DocumentSchemaGroup[], workspaceId: string = getActiveWorkspaceId()) {
  ...
  for (const group of groups) {
    await db.execute("INSERT INTO schema_groups (id, workspace_id, name, description) VALUES (?, ?, ?, ?)", [group.id, scopedWorkspaceId, group.name, group.description ?? null])
    for (const schema of group.documents) {
      ...
      if (schema.metadata) {
        await db.execute("INSERT INTO schema_metadata (schema_id, workspace_id, metadata) VALUES (?, ?, ?)", [schema.id, scopedWorkspaceId, JSON.stringify(schema.metadata)])
      }
    }
  }
}
...
async function saveSpecificationRegistry(registry: SpecificationDefinition[], workspaceId: string = getActiveWorkspaceId()) {
  ...
  for (const item of registry) {
    ...
    if (!id) continue
    await db.execute("INSERT OR REPLACE INTO specification_registry (id, workspace_id, name, description) VALUES (?, ?, ?, ?)", [id, scopedWorkspaceId, item.name.trim() || id, item.description?.trim() || null])
  }
}
...
async function approveMergeProposal(proposalId: string, reviewerId?: string, workspaceId: string = getActiveWorkspaceId(), resolution?: MergeResolutionPayload, reviewerDeviceId?: string) {
  ...
  const db = getDb()
  const [proposal] = await db.query("SELECT * FROM merge_queue WHERE workspace_id = ? AND id = ?", [scopedWorkspaceId, proposalId])
  ...
  if (proposal.action === "DELETE") {
    await db.execute(`UPDATE notes SET is_deleted = 1, deleted_by = ?, updated_by = ?, user_id = ?, device_id = ?, updated_at = ?, synced_at = NULL WHERE workspace_id = ? AND id = ?`, [actor.userId, actor.userId, actor.userId, actor.deviceId, now, scopedWorkspaceId, proposal.document_id])
  } else if (proposal.action === "MERGE_DUPLICATE") {
    await db.execute(`UPDATE notes SET parent_id = ?, updated_by = ?, user_id = ?, device_id = ?, updated_at = ?, synced_at = NULL WHERE workspace_id = ? AND parent_id = ?`, [proposal.document_id, actor.userId, actor.userId, actor.deviceId, now, scopedWorkspaceId, proposal.secondary_document_id])
    await db.execute(`UPDATE archival_records SET article_id = ?, device_id = ?, updated_at = ?, synced_at = NULL WHERE workspace_id = ? AND article_id = ?`, [proposal.document_id, actor.deviceId, now, scopedWorkspaceId, proposal.secondary_document_id])
    await db.execute(`UPDATE notes SET is_deleted = 1, deleted_by = ?, updated_by = ?, user_id = ?, device_id = ?, updated_at = ?, synced_at = NULL WHERE workspace_id = ? AND id = ?`, [actor.userId, actor.userId, actor.userId, actor.deviceId, now, scopedWorkspaceId, proposal.secondary_document_id])
    await db.execute(`UPDATE notes SET title = ?, frontmatter = ?, body = ?, updated_by = ?, user_id = ?, device_id = ?, updated_at = ?, synced_at = NULL WHERE workspace_id = ? AND id = ?`, [resolvedTitle, resolvedFrontmatter, resolvedBody, actor.userId, actor.userId, actor.deviceId, now, scopedWorkspaceId, proposal.document_id])
  } else {
    await db.execute(
      `INSERT INTO notes (id, workspace_id, schema_id, parent_id, title, frontmatter, body, created_by, updated_by, user_id, device_id, created_at, updated_at, is_deleted, synced_at)
       VALUES (?, ?, ?, NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, NULL)
